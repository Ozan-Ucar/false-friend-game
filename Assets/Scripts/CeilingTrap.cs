...
public class CeilingTrap : MonoBehaviour
{
    ...
    public string animLevel1 = "LowCeilingTrapAnim";
    public string animLevel2 = "MidCeilingTrapAnim";
    public string animLevel3 = "HighCeilingTrapAnim";
    ...
    public int damage = 1;
    [Tooltip("Der Collider, der bestimmt, wo die Falle Schaden macht. Am besten ein separates leeres Child-Objekt mit einem BoxCollider2D (Is Trigger) erstellen.")]
    public Collider2D damageCollider;
    
    [Header("Timing")]
    [Tooltip("Zeit (in Sekunden) nach dem Klick, bis die Animation startet")]
    public float preFireDelay = 0f;
    [Tooltip("Zeit (in Sekunden) NACH Animationsstart, bis die Falle den Collider aktiviert (Schaden macht)")]
    public float damageStartDelay = 0.2f;
    [Tooltip("Wie lange die Falle Schaden macht")]
    public float damageDuration = 1.0f;
    [Tooltip("Wie oft der Spieler Schaden nimmt, wenn er in der Falle stehen bleibt (in Sekunden)")]
    ...
    private bool animStateExists = false;
    ...
    private void Awake()
    {
        ...
        // Wir suchen alle Collider auf diesem Objekt (für das Anklicken)
        Collider2D[] allColliders = GetComponents<Collider2D>();
        
        foreach (Collider2D col in allColliders)
        {
            // Wir packen ALLE Collider in unsere Klick-Liste, die NICHT die Hitbox sind
        ...
        {
            Debug.LogWarning("CeilingTrap: Es wurde kein DamageCollider zugewiesen! Bitte im Inspector zuweisen.");
        }
    }
    ...
    private void Start()
    {
        if (anim != null)
        {
            ...
            animStateExists = anim.HasState(0, stateHash);
            ...
            {
                Debug.LogWarning("CeilingTrap '" + gameObject.name + "': Animation '" + CurrentAnimationName + "' wurde im Animator NICHT gefunden! Bitte prüfe ob der State-Name im Animator exakt so heißt.", this);
            }
        }
    }
    ...
    private void Update()
    {
        ...
        // 2. Kontinuierlich auf Schaden prüfen, wenn die Falle aktiv ist
        if (isDamageActive && damageCollider != null)
        {
            // Finde alle Collider, die sich in der Falle befinden
            ...
            foreach (Collider2D col in results)
            {
                if (col.CompareTag("Player"))
                {
                    ...
                    // Sofortiger Schaden, wenn der Timer abgelaufen ist oder man neu in die Falle läuft
                    if (damageTimer <= 0f)
                    {
            ...
            // Wenn der Spieler aus der Falle geht, setze den Timer sofort zurück
    ...
    private IEnumerator FireSequence()
    {
        ...
        // 1. Animation starten
        if (anim != null && animStateExists)
        {
            anim.speed = 1;
            anim.Play(CurrentAnimationName, 0, 0f);
        }

        // WICHTIG: Einen Frame warten!
        yield return null; 
        ...
        if (!animEnded && anim != null && animStateExists)
        {
            anim.Play(CurrentAnimationName, 0, 0f);
            anim.speed = 0;
        }
